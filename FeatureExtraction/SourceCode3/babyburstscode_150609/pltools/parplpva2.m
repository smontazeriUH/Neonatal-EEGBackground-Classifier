function [p,gof]=parplpva2(x, xmin, varargin)
% PARPLPVA2 is a parallized version of PLPVA. 
%
% THIS VERSION REQUIRES THE PARALLEL TOOLBOX TO BE INSTALLED!
%
% Two changes have been made:
% 1) Each forloop has been replaced by a parforloop.
% 2) In the INTS case of the switch, the overhead resulting from
%    calling repmat numerous times has been replaced by a much faster
%    alternative.
%
% Using this version produces warnings when running the file. However
% these warnings are irrelevant w.r.t the calculations and their accuracy.
% 
% The speedup of using this version is proportional to the number of cores
% available on the computer. As an example running the terroristattack
% dataset (see http://www.santafe.edu/~aaronc/powerlaws/ for information on
% how to obtain the set) using the default setup (1000 reps) in the 
% single-threaded manner takes about 9 minutes on a 2.0 Ghz Windows Xp
% 32-bit machine.
% Using PARPLPVA2 on an 8-core machine reduces the computational time to
% around a minute.

% The MatLab version used for extending this file is 7.10.0 (R2010a).

% The following example assumes the user has obtained the terroristattacks
% dataset (>> refers to the MatLab terminal):
% >> [alpha, xmin, L] = plfit(terroristattacks);
% >> matlabpool
% >> [p, gof] = parplpva2(terroristattacks,xmin);
%
% - Using 'matlabpool' readys the computer to use all available cores 
%   (called labs) for processing. If you do not intialize the cores this
%   way, this version is identical to the original i.e. single-threaded. 
%   See 'doc matlabpool' for more information on how to use and close the
%   connection to the cores.
%
% All credit goes to Aaron Clauset for making the original file (See
% below). 
...
nof = zeros(Bt,1);
% Probably not necessary but nevertheless...
nof_length = length(nof);
...
switch f_dattype,
    ...
    case 'REAL',
        ...
        parfor B=1:length(nof)
            ...
            if ~quiet,
                % This is commented out for two very good reasons.
                % 1) It prevents the use of the parfor loop as nof is
                % restricted.
                % 2) Printing iteration progression when using parfor loops is very
                % uninformative.
            ...
            for qm=1:length(qmins)
                ...
                if nq>1
                    try
                        ...
                        % The repmat approach is slow when repmat is called
                        % repeatedly. It has been replaced by a more
                        % efficient non-matlab-script approach.
                        idx   = (1:qmin-1)';
                        zdiff =  sum( idx(:, ones(1,ln)).^-vec(ones(qmin-1,1),:) ,1);
                    ...
                    fit = cumsum((((qmin:qmax).^-vec(I)))./ (zvec(I) - sum(idx'.^-vec(I)))); % changed (1:qmin-1) to idx'
