function [alpha, lambda, xmin, L]=powerexp_fit_xmin(x, varargin)
% powerexp_fit_xmin fits a power law with exponential cutoff and determines
% the xmin above which the fit holds (i.e., finds the start of the tail)
%
%
% Based directly on plfit.m
...
% estimate xmin, alpha, and lambda accordingly
switch f_dattype,
    ...
    case 'REAL',
        ...
        for xm=1:length(xmins)
            ...
            z    = z(z>=xmin); 
            n    = length(z);
            % estimate alpha and lambda using MLE optimization
            testfit=powerexp_fit(z,xmin);
            a=testfit.exponent;
            lambda=testfit.rate;
            %a    = n ./ sum( log(z./xmin) );
            if nosmall,  % leave this?
                if (a-1)/sqrt(n) > 0.1
                    dat(xm:end) = [];
                    xm = length(xmins)+1;
                    break;
            ...
            if isnan(a)
                dat(xm)=NaN; % fit failed
            else
                % compute KS statistic
                cx   = (0:n-1)'./n;
                %cf   = 1-(xmin./z).^a;
                cf=ppowerexp(z,xmin,a,lambda,1,0);
                dat(xm) = max( abs(cf-cx) );
            end
        end;
        %figure, semilogx(xmins,dat)
        D     = min(dat);
        xmin  = xmins(find(dat<=D,1,'first'));
        z     = x(x>=xmin);
        n     = length(z); 
        %alpha = 1 + n ./ sum( log(z./xmin) );
        fit=powerexp_fit(z,xmin);
        alpha=fit.exponent;
        lambda=fit.rate;
        if finite, alpha = alpha*(n-1)/n+1/n; end; % finite-size correction - need this?
        if n < 50 && ~finite && ~nowarn,
%             fprintf('(PLFIT) Warning: finite-size bias may be
%             present.\n'); % commented by saeed
        end;
        %L = n*log((alpha-1)/xmin) - alpha.*sum(log(z./xmin));
        L=powerexp_loglike(z,xmin,alpha,lambda);
...
%             [Y,I] = max(L);
            ...
            % fit discrete power law with exp cutoff
            testdiscfit=discpowerexp_fit(z,xmin);
            
            % compute KS statistic
            %fit = cumsum((((xmin:xmax).^-vec(I)))./ (zvec(I) - sum((1:xmin-1).^-vec(I))));
            cf=pdiscpowerexp(z,a,lambda,xmin,1,0); % NOT YET WRITTEN
            dat(xm) = max( abs(cf-cx) );
